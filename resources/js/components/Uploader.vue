    ...
    <label :class="{'disabled': isBrowseDisabled}">
      <span v-if="isSingleFileUpload">
        ...
        <slot name="browse-btn">
          <span class="vuejs-uploader__btn">Add Photo</span>
        </slot>
      ...
      <span v-if="isMultipleFileUpload">
        <slot name="browse-btn">
          <span class="vuejs-uploader__btn">Add Photos</span>
        </slot>
      </span>
      <!-- File Input -->
      <input type="file" :disabled="isBrowseDisabled" :multiple="multiple" :accept="accept" @change="selectFiles" accept="image/*" capture="environment">
    </label>
