    ...
    <div>
            ...
            <table class="table table-striped table-hover mb-0">
                <thead>
                    <tr>
                        <ss-grid-column-header
                            ...
                            title="Sort by Legacy Charge Code"
                            :params="{
                                sortField: 'legacy_charge_code',
                        ...
                        <ss-grid-column-header
                            v-on:selectedSort="sortColumn"
                            v-bind:selectedKey="sortKey"
                            title="Sort by Short Description"
                            :params="{
                        ...
                        <ss-grid-column-header
                            v-on:selectedSort="sortColumn"
                            v-bind:selectedKey="sortKey"
                            title="Sort by Inactive Date"
                            :params="{
                                sortField: 'inactive_date',
                                InitialSortOrder: 'asc'
                            }"
                        >
                            Inactive 
                ...
                <tbody>
                    ...
                    <tr v-else v-for="row in this.gridData" :key="row.id">
                        <td data-title="Legacy Charge Code">
                            {{ row.legacy_charge_code }}
                        </td>
                        <td data-title="Charge Code">{{ row.charge_code }}</td>

                        <td data-title="Short Description">
                            {{ row.short_description }}
                        </td>
                        <td data-title="Charge Type">{{ row.charge_type }}</td>
                        <td data-title="Classification">
                            {{ row.classification }}
                        </td>
                        <td data-title="Effective Date">
                            <dsp-date v-model="row.effective_date"></dsp-date>
                        </td>
                        <td data-title="Inactive Date">
                            <dsp-date v-model="row.inactive_date"></dsp-date>
                        </td>
        ...
        <div class="grid-bottom row mb-0 align-items-center">
            <div class="col-lg-4 mb-2">
                <a
                    href="/import-mshp-charge-code/download"
                    class="btn btn-primary mb-2 mr-2"
                    >Export to Excel</a
                >
                <a
                    href="/import-mshp-charge-code/print"
                    class="btn btn-primary mb-2 mr-2"
                    >Print PDF</a
                >
            </div>
...
export default {
    name: "import-mshp-charge-code-grid",
    components: {
        SsGridColumnHeader,
    ...
    methods: {
        goToNew: function() {
            window.location.href = "/import-mshp-charge-code/create";
        },
        ...
        getDataUrl: function(new_page_number) {
            var url = "api-import-mshp-charge-code?";
            var queryParams = [];
